...
fdiv => {
	...
	constructors => {
		...
		d => { in_reqs => [ "cls-fp", "cls-fp" ], out_reqs => [ "cls-fp", "mem" ] },
...
fftof => {
	...
	constructors => {
		s_d => { in_reqs => [ "cls-fp"   ], out_reqs => [ "cls-fp" ], mode => $mode_fp2, },
		s_q => { in_reqs => [ "cls-fp"   ], out_reqs => [ "cls-fp" ], mode => $mode_fp4, },
		d_s => { in_reqs => [ "cls-fp" ], out_reqs => [ "cls-fp"   ] },
		d_q => { in_reqs => [ "cls-fp" ], out_reqs => [ "cls-fp:4" ], mode => $mode_fp4, },
		q_s => { in_reqs => [ "cls-fp:4" ], out_reqs => [ "cls-fp"   ] },
		q_d => { in_reqs => [ "cls-fp:4" ], out_reqs => [ "cls-fp" ], mode => $mode_fp2, },
	},
},
...
fitof => {
	...
	constructors => {
		s => { out_reqs => [ "cls-fp"   ] },
		d => { out_reqs => [ "cls-fp" ], mode => $mode_fp2, },
